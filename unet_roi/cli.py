"""Command Line Interface for UNET Ultrasound ROI Segmentation."""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="U-Net Ultrasound ROI Segmentation Tool",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Train a new model
  unet-roi train --image-dir data/images --mask-dir data/masks --model-path models/my_model.keras

  # Make predictions on images
  unet-roi predict --model-path models/unet_EchoRoi.keras --input data/test_image.png --output results/
...
def train_cli(args):
    ...
    print("="*60)
    print("UNET ULTRASOUND ROI SEGMENTATION - TRAINING")
    print("="*60)
    ...
    # Train model
    history = trainer.train(args.image_dir, args.mask_dir, args.model_path)
    
    # Plot training history
    plot_path = os.path.join(os.path.dirname(args.model_path), "training_history.png")
...
def predict_cli(args):
    ...
    print("="*60)
    print("UNET ULTRASOUND ROI SEGMENTATION - PREDICTION")
    print("="*60)
...
def evaluate_cli(args):
    ...
    from .training import calculate_metrics, evaluate_model
    from .preprocessing import UltrasoundPreprocessor
    import json
    
    print("="*60)
    print("UNET ULTRASOUND ROI SEGMENTATION - EVALUATION")
    print("="*60)
...
def create_data_cli(args):
    ...
    print("="*60)
    print("UNET ULTRASOUND ROI SEGMENTATION - CREATE SAMPLE DATA")
    print("="*60)
...
def benchmark_cli(args):
    ...
    print("="*60)
    print("UNET ULTRASOUND ROI SEGMENTATION - BENCHMARK")
    print("="*60)
...
def train_cli_entry():
    ...
    import sys
    sys.argv = ['unet-roi', 'train'] + sys.argv[1:]
    main()
...
def predict_cli_entry():
    ...
    import sys
    sys.argv = ['unet-roi', 'predict'] + sys.argv[1:]
    main()
...
def evaluate_cli_entry():
    ...
    import sys
    sys.argv = ['unet-roi', 'evaluate'] + sys.argv[1:]
    main()
